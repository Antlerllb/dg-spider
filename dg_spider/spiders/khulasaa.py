...
class KhulasaaSpider(BaseSpider):
    ...
    def parse_category1(self, response):
        ...
        LastTimeStamp = OldDateUtil.format_time3(str(khulasaa_time_switch(BeautifulSoup(requests.get(news_url[-1]).text, features="lxml").select_one(".author-disc .date .author span").text)))
        if next_page:
            ...
            elif LastTimeStamp < OldDateUtil.time:
