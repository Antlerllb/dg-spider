...
class AccmSpider(BaseSpider):
    ...
    def parse_list(self, response):
        ...
        pub = time.strftime("%Y-%m-%d %H:%M:%S", datetime(int(pub[-1]), OldDateUtil.month[pub[1]], int(pub[0])).timetuple())
        if OldDateUtil.time == None or OldDateUtil.format_time3(pub) >= int(OldDateUtil.time):
            for i in soup.find_all(class_='article-thumb'):
                ...
                response.meta['pub_time'] = time.strftime("%Y-%m-%d %H:%M:%S", datetime(int(pub_time[-1]), OldDateUtil.month[pub_time[1]], int(pub_time[0])).timetuple())
