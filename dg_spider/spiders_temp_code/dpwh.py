...
class DemoSpiderSpider(BaseSpider):
    ...
    def parse_page(self,response):
        ...
        if response.meta['category1'] == 'News':
            ...
            last_time = t[2] + '-' + str(OldDateUtil.month[t[0]]).rjust(2,'0') + '-' + t[1].rjust(2,'0') + " 00:00:00"
        else:
            t = news_page[0].select('div')[3].select('span')[1].text.split(' ')
            last_time=t[4] + '-' + str(OldDateUtil.month[t[3]]).rjust(2,'0') + '-' + t[1].replace('th', '').replace('st', '').replace('nd', '').rjust(2,'0')+ " 00:00:00"
        if OldDateUtil.time is None or int(OldDateUtil.time)<OldDateUtil.str_datetime_to_timestamp(last_time):
            for i in news_page:
                if response.meta['category1']=='News':
                    ...
                    response.meta['time'] = t[2] + '-' + str(OldDateUtil.month[t[0]]).rjust(2,'0') + '-' + t[1] .rjust(2,'0')+ " 00:00:00"
                else :
                    t = i.select('div')[3].select('span')[1].text.split(' ')
                    response.meta['time']= t[4] + '-' + str(OldDateUtil.month[t[3]]).rjust(2,'0') + '-' + t[1].replace('th', '').replace('st', '').replace('nd', '').rjust(2,'0') + " 00:00:00"
