...
class china_consulateSpider(BaseSpider):
    ...
    def parse_category(self, response, index=glo_index):
        ...
        if OldDateUtil.time is None or OldDateUtil.format_time3(re.search("[0-9]{4}-[0-9]{2}-[0-9]{2}", soup.select("div.nbox_ul a")[-1].text).group() + " 00:00:00") >= int(OldDateUtil.time):
            article_hrefs = []
        ...
        # else:
        #     index = 1
        #     if OldDateUtil.time is None or OldDateUtil.format_time3(soup.select("ul[style=\"margin-left:5px\"] li span")[-1].text.replace("(", "").replace(")", "") + " 00:00:00") >= int(OldDateUtil.time):
        #         next_page = re.search("^.*/", response.url).group() + "default_" + str(index) + ".htm"
        #         index = index + 1
        #         meta = {"index": index}
