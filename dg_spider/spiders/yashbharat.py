...
class yashbharat(BaseSpider):
    ...
    def get_next_page(self, response):
            ...
            if OldDateUtil.time == None or OldDateUtil.format_time3(OldDateUtil.format_time2(soup.select('.article-container > article time')[-1].text)) >= int(OldDateUtil.time):
    ...
    def get_news_detail(self,response):
        ...
        pub_time = OldDateUtil.format_time2(soup.find("time", class_="entry-date published updated").text) if soup.find("time",class_="entry-date published updated") else None
