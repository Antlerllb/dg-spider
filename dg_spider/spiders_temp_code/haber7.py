...
def last_time(time):
    ...
    if "DAKİKA ÖNCE" in time: # 分钟前
        la_time = int(OldDateUtil.get_time_ago_stamp(minute=int(l_time)))
    elif "SAAT ÖNCE" in time: # 小时前
        la_time = int(OldDateUtil.get_time_ago_stamp(hour=int(l_time)))
    elif "GÜN ÖNCE" in time: # 天前
        la_time = int(OldDateUtil.get_time_ago_stamp(day=int(l_time)))
    elif "HAFTA ÖNCE" in time: # 周前
        la_time = int(OldDateUtil.get_time_ago_stamp(week=int(l_time)))
    elif "AY ÖNCE" in time: # 月前
        la_time = int(OldDateUtil.get_time_ago_stamp(month=int(l_time)))
    else: # 年前
        la_time = int(OldDateUtil.get_time_ago_stamp(year=int(l_time)))
    return la_time
    ...
    def parse_page(self,response): #分类较多
        ...
        if response.meta["category1"] == "Spor Haberleri" and response.meta["category2"] == "None":  # 有再分类
            ...
            try:
                ...
                if OldDateUtil.time is None or last_time_ >= int(OldDateUtil.time):
