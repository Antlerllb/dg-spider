...
class krishakjagatSpider(BaseSpider):
    ...
    def get_next_page(self, response):
            ...
            if OldDateUtil.time == None or OldDateUtil.format_time3(OldDateUtil.format_time2(soup.find("div",class_="article-container").find_all("time",class_="entry-date published")[-1].text.strip())) >= int(OldDateUtil.time):
    ...
    def get_news_detail(self,response):
        ...
        pub_time = OldDateUtil.format_time2(soup.find("article").find("time", class_="entry-date published").text)
