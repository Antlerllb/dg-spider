...
class tehelkahindiSpider(BaseSpider):
    ...
    def get_next_page(self, response):
            ...
            for a in a_list:
                ...
                if OldDateUtil.time == None or OldDateUtil.format_time3(OldDateUtil.format_time2(soup.find_all("time",class_="entry-date updated td-module-date")[-1].text)) >= int(OldDateUtil.time):
    ...
    def get_news_detail(self,response):
        ...
        pub_time = OldDateUtil.format_time2(soup.find("time", class_="entry-date updated td-module-date").text)
