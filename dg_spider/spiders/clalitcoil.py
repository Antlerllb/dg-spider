        ...
        if div_obj_list:  # 如果新闻列表有新闻
            ...
            if OldDateUtil.time is None:
                for div_obj in div_obj_list:
                    response.meta['pub_time'] = time.strftime('%Y-%m-%d %H:%M:%S', time.strptime(div_obj.css('p.article-date::text').re('\d+/\d+/\d+')[0],"%d/%m/%Y")) if div_obj.css('p.article-date') else OldDateUtil.get_now_datetime_str()
