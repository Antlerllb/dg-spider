...
class coconuts(BaseSpider):# cyl
    ...
    def parse_category4(self, response):
        if response.status == int(200):
            ...
            if details:
                ...
                ddl = OldDateUtil.format_time2(ddl)
                ddl = OldDateUtil.format_time3(ddl)
            else:
                ddl = None
            if (OldDateUtil.time == None or (ddl != None and ddl >= int(OldDateUtil.time))):
    ...
    def parse_category5(self, response):
        ...
        if html.select_one("time.post-timeago ") is not None:
            ...
            pub_time = OldDateUtil.format_time2(pub_time)
