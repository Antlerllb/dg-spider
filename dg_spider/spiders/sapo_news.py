...
class sapo_news(BaseSpider):
        ...
        if len(soup.select('section > ul > li'))>2:
            for i in soup.select('section > ul > li'):
                ...
                if 'https' in i.select_one('a').get('href'):
                    if OldDateUtil.time == None or int(t) >= OldDateUtil.time or t==None:
                        # print(OldDateUtil.time_stamp2formate_time(t))
    ...
    def essay_parser(self,response):
        ...
        #格式化时间
        try:
            time = OldDateUtil.timestamp_to_datetime_str(time)
        except:
