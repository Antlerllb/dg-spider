...
class unboxSpider(BaseSpider):
    ...
    def start_requests(self):
        ...
        for i in range(1,5000):
            ...
            if OldDateUtil.time != None and OldDateUtil.format_time3(OldDateUtil.format_time2(html.select('li .date.meta-item.tie-icon')[-1].text)) < int(OldDateUtil.time):
                break
    ...
    def parse(self, response):
        ...
        item['pub_time'] = OldDateUtil.format_time2(html.select('span[class="date meta-item tie-icon"]')[0].text)
        item['images'] = []
