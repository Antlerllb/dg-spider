...
class MyanmarisisSpider(BaseSpider):
    ...
    def parse(self, response):   # 新闻列表 有完整新闻
        ...
        flag = True
        last_pub_time= OldDateUtil.format_time2(soup.select('#page-content-wrapper > div.container.section-1 > div > div.col-lg-9.col-md-9.col-sm-9.col-xs-12 > div > div h5')[-1].text)
        if OldDateUtil.time is None or OldDateUtil.format_time3(last_pub_time) >= int(OldDateUtil.time):
            all_pub_time = [OldDateUtil.format_time2(i.text) for i in soup.select('#page-content-wrapper > div.container.section-1 > div > div.col-lg-9.col-md-9.col-sm-9.col-xs-12 > div > div h5')]
