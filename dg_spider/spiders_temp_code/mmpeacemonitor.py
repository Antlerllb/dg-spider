...
class MmpeacemonitorSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        flag=True
        last_pub=OldDateUtil.format_time2(soup.select('.author>li')[-2].text.split(':')[1].replace('th',''))
        ...
        if OldDateUtil.time is None or OldDateUtil.format_time3(last_pub) >= int(OldDateUtil.time):
            ...
            for i in soup.select('h2>a'):
                meta={'title':i.text.strip(),'pub_time':OldDateUtil.format_time2(all_pub_time[j])}
                j+=1
