...
class philnewsSpider(BaseSpider):
    ...
    def parse2(self, response):
        ...
        if html.select('.previous_posts > a') != [] and (OldDateUtil.time == None or OldDateUtil.format_time3(OldDateUtil.format_time2(html.select('div[class="default-post-category-content post_box"] .post_date')[-1].text)) >= int(OldDateUtil.time)):
    ...
    def parse3(self, response):
        try:
            ...
            item['pub_time'] = OldDateUtil.format_time2(html.select('.post_date_intro > .published')[0].text)
            item['images'] = []
