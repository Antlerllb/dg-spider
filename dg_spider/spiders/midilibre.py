...
class MidilibreSpider(BaseSpider):
    name = 'midilibre'
    author = '潘宇豪'
    website_id = 2771
    language_id = 1884
    current_day = ''
    begin = False
    start_urls = ['https://www.midilibre.fr/']
    ...
    def get_today_url():
        ...
        MidilibreSpider.current_day = year + '-' + month + '-' + day + ' 00:00:00'
        return 'https://www.midilibre.fr/articles/' + year + '/' + month + '/' + day

    @staticmethod
    def get_yesterday_url(today):
        MidilibreSpider.current_day = OldDateUtil.timestamp_to_datetime_str(
            OldDateUtil.str_datetime_to_timestamp(today) - 86400)
        yesterday = MidilibreSpider.current_day.split(' ')[0]
        return 'https://www.midilibre.fr/articles/' + yesterday.split('-')[0] + '/' + yesterday.split('-')[1] + '/' + \
            yesterday.split('-')[2]
            ...
            if OldDateUtil.time is None or OldDateUtil.time < OldDateUtil.str_datetime_to_timestamp(self.current_day):
                for category in category_list:
                    ...
                    for new in news:
                        ...
                        # print('    ' + title)
                        detailed_url = 'https://www.midilibre.fr/' + new.find('a')['href']
                        # print('    ' + detailed_url)
