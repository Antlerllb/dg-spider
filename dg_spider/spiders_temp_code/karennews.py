...
class KarennewsSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        flag = True
        last_pub_time = OldDateUtil.format_time2(soup.select('#masonry-grid > div .date.meta-item.fa-before')[-1].text.strip())
    ...
    def parse_more(self, response):
        ...
        flag = True
        last_pub_time = OldDateUtil.format_time2(soup.select('.date.meta-item.fa-before')[-1].text)
        if OldDateUtil.time is None or OldDateUtil.format_time3(last_pub_time) >= int(OldDateUtil.time):
    ...
    def parse_item(self, response):
        ...
        item['pub_time'] =OldDateUtil.format_time2(soup.select_one('.date.meta-item.fa-before').text)
