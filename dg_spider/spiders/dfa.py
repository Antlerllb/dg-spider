...
class dfaSpider(BaseSpider):
    ...
    def parse_news_list(self, response):
        ...
        if OldDateUtil.time == None or (time2 and OldDateUtil.format_time3(time2) >= int(OldDateUtil.time)):
            if next_page:
    ...
    def parse_news(self, response):
        ...
        item["pub_time"] = response.meta["pub_time"] if response.meta["pub_time"] != None else OldDateUtil.format_time()
        soup = BeautifulSoup(response.text, "html.parser")
