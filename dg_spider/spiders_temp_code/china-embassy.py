...
class ChinaembassySpider(BaseSpider):
    ...
    def next_page(self, response):
        ...
        if OldDateUtil.time is not None:
            t = soup.select('#right > div.center_content > ul > li')[0].text.split('(')[1].strip(')').split('/')
            tt = "{}-{}-{}".format(t[0], t[1], t[2]) + ' 00:00:00'
        if OldDateUtil.time == None or OldDateUtil.str_datetime_to_timestamp(tt) >= OldDateUtil.time:
            for i in range(0, 28):
                if i == 0:
