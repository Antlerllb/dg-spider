...
class MoeaSpider(BaseSpider):
    ...
    def parse_list(self,response):
        ...
        for i in soup.select('div.isotope.row > div'):
            ...
            response.meta['pub_time'] = OldDateUtil.format_time2(pub_time)
            yield Request(url,callback=self.parse_news,meta=response.meta)
        pub = OldDateUtil.format_time2(soup.select('div.isotope.row > div')[-1].select_one('div > div > div > span.date').text.strip())
