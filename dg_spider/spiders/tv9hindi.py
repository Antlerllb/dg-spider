...
class Tv9hindiSpider(BaseSpider):
    ...
    def parse_category1(self, response):
        ...
        last_timeStamp = OldDateUtil.format_time3(OldDateUtil.format_time2(soup.find_all("div", class_="col2 ComListing")[-1].find_all("div", class_="catTime flex")[-1].find("span").text.strip()))
        ...
        if next_page:
            if OldDateUtil.time is None:
                if int(next_page.rsplit("/", 1)[-1]) <= 50:
                    ...
                    pass
            elif last_timeStamp >= int(OldDateUtil.time):
                yield scrapy.Request(next_page, callback=self.parse_category1)
            else:
