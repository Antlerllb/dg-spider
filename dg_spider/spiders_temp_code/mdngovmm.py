...
class MdngovmmSpider(BaseSpider):
    ...
    def parse_more(self, response):  # 动态翻页
        soup = BeautifulSoup(response.text, 'html.parser')
        last_pub = OldDateUtil.format_time2(soup.select('.post-content .post-created')[0].text)
        last_pub_stamp = OldDateUtil.format_time3(last_pub)
        self.logger.info(last_pub)
        flag = True
        if OldDateUtil.time is None or last_pub_stamp >= int(OldDateUtil.time):
            for i in soup.select('.block-content .post-content')[:-5]:
                ...
                response.meta['pub_time'] = OldDateUtil.format_time2(soup.select_one('.post-created').text)
