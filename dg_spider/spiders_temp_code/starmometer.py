...
class starmometer(BaseSpider):
    ...
    def parse_category2(self,response):
        ...
        if soup.select('.loop-data>.meta'):
            ...
            ddl=OldDateUtil.format_time2(ddl[0])#2021-01-10 00:00:00
            ddl=OldDateUtil.format_time3(ddl)#1610208000
        else:
            ddl=None
        if soup.find('a',class_='next page-numbers'):
            next_url=soup.find('a',class_='next page-numbers').get('href')
            if(OldDateUtil.time==None or ddl>=int(OldDateUtil.time)):
    ...
    def parse_details(self,response):
        ...
        if(pub):
            pub=OldDateUtil.format_time2(pub)
            item['pub_time']=pub
