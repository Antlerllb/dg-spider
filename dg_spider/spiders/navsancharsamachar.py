...
class NavsSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        for i in soup.select('.post'):
            ...
            response.meta['pub_time'] = OldDateUtil.format_time2(i.select_one('.date').text)
            if OldDateUtil.time is None or OldDateUtil.format_time3(OldDateUtil.format_time2(i.select_one('.date').text)) >= int(OldDateUtil.time):
