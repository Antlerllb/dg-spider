...
class BoholchronicleSpider(BaseSpider):
    ...
    def parse2(self, response):
        ...
        # 下面这里就是截止方法
        # time_为此页最后一篇文章的发布时间
        time_ = OldDateUtil.format_time3(OldDateUtil.format_time2(html.select('#loop-container div .post-byline')[-1].text))
        # 时间判断，继续爬取下一页的条件是time为None或者time_大于time
        if OldDateUtil.time == None or time_ >= int(OldDateUtil.time):
            # 爬取下一页
    ...
    def parse3(self, response):
        ...
        item['pub_time'] = OldDateUtil.format_time2(html.select('.post-title ~ .post-byline')[0].text)
