...
class ZeenSpider(BaseSpider):
    ...
    def parse_eassys(self, response):  # 各类二级目录的文章的翻页和url爬取
        ...
        if re.match(r'.*photo-gallery.*', response.url):  # 照片的
            for t in soup.find_all(class_='col-sm-4 col-md-4 photo-photo-h'):
                ...
                if OldDateUtil.time is None or OldDateUtil.format_time3(OldDateUtil.format_time2(t.select_one('.photo-date').text.strip())) >= int(OldDateUtil.time):
            ...
            for i in soup.find_all(attrs={'class': 'mini-video mini-video-h margin-bt30px'}):  # 该目录初始的文章
                ...
                if OldDateUtil.time is None or OldDateUtil.format_time3(OldDateUtil.format_time2(i.select_one('span.date').text.strip())) >= int(OldDateUtil.time):
            ...
            for t in soup.find_all(class_='section-article margin-bt30px clearfix'):  # 该目录初始的文章
                ...
                response.meta['images'] = [t.select_one('img').get('src')]
                if OldDateUtil.time is None or OldDateUtil.format_time3(OldDateUtil.format_time2(pub_time)) >= int(OldDateUtil.time):
    ...
    def parse_item(self, response):
        ...
        item['pub_time'] = OldDateUtil.format_time2(response.meta['pub_time'])
